...
const Hero = () => {
  ...
  return (
    <>
 {openVideoModal && (
  <div onClick={() => setOpenVideoModal(false)} className="fixed inset-0 transition-opacity duration-300 z-50 bg-black/80 flex items-center justify-center p-4">
    {/* Close Button */}


    {/* YouTube Video */}
    <div onClick={(e) => {e.preventDefault()}} className="relative w-full max-w-3xl aspect-video rounded-xl overflow-hidden shadow-lg">
            <iframe
        className="w-full h-full"
        src="https://youtube.com/shorts/ugBTo_Qxma8?si=cLpYMPrq0geBSp_l"
        title="Demo Video"
      ...
      <div className="min-h-screen pb-20">
        ...
        <nav className="z-50 flex items-center justify-between w-full py-4 px-6 md:px-16 lg:px-24 xl:px-40 text-sm">
          <Link to="/">
            <img src="/favicon.svg" alt="logo" className="h-11 w-auto" />
          </Link>

          <div className="hidden md:flex items-center gap-8 transition duration-500 text-slate-800">
            <a href="#" className="hover:text-green-600 transition">Home</a>
            <a href="#features" className="hover:text-green-600 transition">Features</a>
            <a href="#testimonial" className="hover:text-green-600 transition">Testimonials</a>
          ...
          <div className="flex gap-2">
            {isSignedIn ? (
              ...
              <SignInButton mode="modal">
                <button className="hidden md:block px-6 py-2 border active:scale-95 hover:bg-slate-50 transition-all rounded-full text-slate-700 hover:text-slate-900">
                  Login
                </button>
              </SignInButton>
            )}
